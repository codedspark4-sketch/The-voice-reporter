...
const parser = new Parser({
  ...
  headers: {
    "User-Agent":
      "The Voice Reporter News Desk/1.0 (+live-news-aggregator)"
  }
});
 ...
 * ============================================================
 * THE VOICE REPORTER
 * LIVE RSS NEWS SERVICE + INTERNAL ARTICLE READER
 * ============================================================
 */
...
const FEEDS = [
  ...
  },
  {
    ...
    region: "Nigeria",
    category: "Politics",
    url: "https://rss.punchng.com/v1/category/politics"
  },
  {
    name: "PUNCH Business",
    region: "Nigeria",
    category: "Business",
    url: "https://rss.punchng.com/v1/category/business"
  },
  {
    name: "PUNCH Sports",
    region: "Nigeria",
    category: "Sports",
    url: "https://rss.punchng.com/v1/category/sports"
  },
  {
    name: "Premium Times",
    region: "Nigeria",
  ...
  },
  {
    ...
    region: "World",
    category: "World",
    url: "https://www3.nhk.or.jp/rssxml/news/globalnewsroom.xml"
  },
  {
    name: "CBC World",
    region: "World",
...
function inferCategory(feed, item) {
  const text =
    `${feed.category} ${item.categories || ""} ${item.title || ""}`.toLowerCase();

  if (feed.category !== "Nigeria" && feed.category !== "World") {
    return feed.category;
  }

  if (
    /sport|football|soccer|tennis|basketball|olympic|cricket|rugby/.test(
      text
    )
  ) {
    return "Sports";
  ...
  if (
    /business|econom|market|finance|bank|oil|trade|stock|inflation/.test(
      text
    )
  ) {
    return "Business";
  ...
  if (
    /technology|tech|artificial intelligence|ai\b|cyber|software|gadget/.test(
      text
    )
  ) {
    return "Technology";
  ...
  if (
    /politic|election|president|senate|governor|minister|government|parliament/.test(
      text
    )
  ) {
    return "Politics";
...
async function refreshNews() {
  ...
  console.log(
    `[news] ${all.length} unique stories from ${Object.keys(
      sources
    ).length} built-in sources`
  );
}
...
async function fetchArticle(url) {
  ...
  const response = await fetch(url, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (compatible; The Voice Reporter Reader/1.0)"
    },
    redirect: "follow"
  });
...
app.get("/api/news", (req, res) => {
  const limit = Math.min(
    Math.max(
      parseInt(req.query.limit || "160", 10),
      1
    ),
    300
  );
...
app.get("/api/article", async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({
      ok: false,
      error:
        "This article could not be read automatically. You can open the original publisher instead."
    });
  }
});
...
app.get("/api/health", (_req, res) => {
  res.json({
    ok: true,
    service:
      "THE VOICE REPORTER live news service",
...
async function boot() {
  ...
  app.listen(
    ...
    () => {
      console.log(
        `THE VOICE REPORTER running on 0.0.0.0:${PORT}`
      );
    }
  );
}
